<script>
    ...
    import { BarsSolid, CloseSolid, LinkedinSolid } from 'flowbite-svelte-icons'

    let sidebarEnabled = false;
</script>


<div id='NavBar' class='fixed w-full h-20 md:h-16 shadow-lg bg-black bg-opacity-25 flex justify-between items-center text-white'>
    <!-- Dark mode button -->
    <DarkMode/>
    ...
    <!-- Hamburger -->
    <button class='md:hidden p-2 mr-8' on:click={()=> sidebarEnabled = true}>
        <BarsSolid size='xl'/>
    </button>
</div>

<!-- Mobile nav -->
<div id='Nav-mobile' class='{sidebarEnabled ? '' : 'hidden'} animate-appear absolute top-0 left-0 w-screen h-screen bg-black bg-opacity-25 text-white flex flex-col justify-center items-center' on:click={()=> sidebarEnabled = false}>
    <!-- Close button -->
...
<!-- Icons -->
<!-- <div id='NavBar-socials' class='fixed bg-green-4 text-white'>
    <LinkedinSolid/>
</div> -->
