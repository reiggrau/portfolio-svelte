...
{#if $overlay}
	<div
		id="LoadingOverlay"
		class="absolute w-screen h-screen top-0 left-0 z-50 bg-black flex flex-col justify-center items-center {overlayDisappears
			? 'animate-disappearVerySlow'
			: ''}"
	>
		<div class="flex flex-col gap-4 {loadingDisappears ? 'animate-disappear' : 'animate-appear'}">
			<h3 class="text-white text-2xl font-extralight">
				<span class="font-normal text-3xl"> ReigGrau </span>
				presents:
			</h3>
			{#if continueReady}
			...
			{:else}
				<div class="px-[20px] py-[10px] h-[50px] flex justify-center">
					...
					<span class="absolute blur text-[#03e9f4] tracking-[4px] animate-appearVerySlow"
						>LOADING...</span
					>
				</div>
			{/if}
